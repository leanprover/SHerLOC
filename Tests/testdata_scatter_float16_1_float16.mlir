"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1xf16>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1xf16>, tensor<f16>)
    ...
    }) : (tensor<1xf16>, tensor<1xi64>, tensor<f16>) -> tensor<1xf16>
  ...
  "func.func"() <{function_type = () -> (tensor<1xf16>, tensor<f16>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<2.248050e+00> : tensor<1xf16>}> : () -> tensor<1xf16>
    %2 = "stablehlo.constant"() <{value = dense<-9.038080e-01> : tensor<f16>}> : () -> tensor<f16>
    "func.return"(%1, %2) : (tensor<1xf16>, tensor<f16>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1xf16>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<-9.038080e-01> : tensor<1xf16>}> : () -> tensor<1xf16>
