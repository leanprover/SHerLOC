"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1xi8>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1xi8>, tensor<i8>)
    ...
    }) : (tensor<1xi8>, tensor<1xi64>, tensor<i8>) -> tensor<1xi8>
  ...
  "func.func"() <{function_type = () -> (tensor<1xi8>, tensor<i8>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<2> : tensor<1xi8>}> : () -> tensor<1xi8>
    %2 = "stablehlo.constant"() <{value = dense<-5> : tensor<i8>}> : () -> tensor<i8>
    "func.return"(%1, %2) : (tensor<1xi8>, tensor<i8>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1xi8>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<-10> : tensor<1xi8>}> : () -> tensor<1xi8>
