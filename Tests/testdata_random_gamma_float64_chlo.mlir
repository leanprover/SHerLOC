"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<f64>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %528 = "func.call"() <{callee = @inputs}> : () -> tensor<f64>
    %529 = "func.call"() <{callee = @expected}> : () -> tensor<f64>
    %530 = "func.call"(%528) <{callee = @"<lambda>"}> : (tensor<f64>) -> tensor<f64>
    "stablehlo.custom_call"(%530, %529) <{call_target_name = "check.expect_close", has_side_effect = true}> : (tensor<f64>, tensor<f64>) -> ()
    "func.return"(%530) : (tensor<f64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<f64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %527 = "stablehlo.constant"() <{value = dense<0.024558972542420593> : tensor<f64>}> : () -> tensor<f64>
  ...
  "func.func"() <{function_type = () -> tensor<f64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %526 = "stablehlo.constant"() <{value = dense<6.1163297886932108E-4> : tensor<f64>}> : () -> tensor<f64>
    "func.return"(%526) : (tensor<f64>) -> ()
  }) : () -> ()
  "func.func"() <{arg_attrs = [{mhlo.layout_mode = "default"}], function_type = (tensor<f64>) -> tensor<f64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "<lambda>", sym_visibility = "private"}> ({
  ^bb0(%arg165: tensor<f64>):
    %514 = "stablehlo.constant"() <{value = dense<42> : tensor<i64>}> : () -> tensor<i64>
    %515 = "stablehlo.constant"() <{value = dense<32> : tensor<i64>}> : () -> tensor<i64>
    %516 = "stablehlo.shift_right_logical"(%514, %515) : (tensor<i64>, tensor<i64>) -> tensor<i64>
    %517 = "stablehlo.convert"(%516) : (tensor<i64>) -> tensor<ui32>
    %518 = "stablehlo.broadcast_in_dim"(%517) <{broadcast_dimensions = array<i64>}> : (tensor<ui32>) -> tensor<1xui32>
    %519 = "stablehlo.constant"() <{value = dense<4294967295> : tensor<ui32>}> : () -> tensor<ui32>
    %520 = "stablehlo.convert"(%519) : (tensor<ui32>) -> tensor<i64>
    %521 = "stablehlo.and"(%514, %520) : (tensor<i64>, tensor<i64>) -> tensor<i64>
    %522 = "stablehlo.convert"(%521) : (tensor<i64>) -> tensor<ui32>
    %523 = "stablehlo.broadcast_in_dim"(%522) <{broadcast_dimensions = array<i64>}> : (tensor<ui32>) -> tensor<1xui32>
    %524 = "stablehlo.concatenate"(%518, %523) <{dimension = 0 : i64}> : (tensor<1xui32>, tensor<1xui32>) -> tensor<2xui32>
    %525 = "func.call"(%524, %arg165) <{callee = @_gamma}> : (tensor<2xui32>, tensor<f64>) -> tensor<f64>
    "func.return"(%525) : (tensor<f64>) -> ()
  }) : () -> ()
  "func.func"() <{arg_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], function_type = (tensor<2xui32>, tensor<f64>) -> tensor<f64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "_gamma", sym_visibility = "private"}> ({
  ^bb0(%arg155: tensor<2xui32>, %arg156: tensor<f64>):
    %475 = "stablehlo.reshape"(%arg155) : (tensor<2xui32>) -> tensor<1x2xui32>
    %476 = "func.call"(%475) <{callee = @_threefry_split}> : (tensor<1x2xui32>) -> tensor<1x1x2xui32>
    %477 = "stablehlo.reshape"(%476) : (tensor<1x1x2xui32>) -> tensor<1x2xui32>
    %478 = "stablehlo.reshape"(%arg156) : (tensor<f64>) -> tensor<1xf64>
    %479 = "stablehlo.constant"() <{value = dense<0.000000e+00> : tensor<f64>}> : () -> tensor<f64>
    %480 = "stablehlo.broadcast_in_dim"(%479) <{broadcast_dimensions = array<i64>}> : (tensor<f64>) -> tensor<1xf64>
    %481 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
    %482:4 = "stablehlo.while"(%477, %478, %481, %480) ({
    ^bb0(%arg161: tensor<1x2xui32>, %arg162: tensor<1xf64>, %arg163: tensor<i64>, %arg164: tensor<1xf64>):
      ...
      %513 = "stablehlo.compare"(%arg163, %512) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      "stablehlo.return"(%513) : (tensor<i1>) -> ()
    }, {
    ^bb0(%arg157: tensor<1x2xui32>, %arg158: tensor<1xf64>, %arg159: tensor<i64>, %arg160: tensor<1xf64>):
      ...
      %485 = "stablehlo.compare"(%arg159, %484) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %486 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %487 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
      %488 = "stablehlo.add"(%486, %487) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %489 = "stablehlo.select"(%485, %488, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %490 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %491 = "stablehlo.dynamic_slice"(%arg157, %489, %490) <{slice_sizes = array<i64: 1, 2>}> : (tensor<1x2xui32>, tensor<i64>, tensor<i64>) -> tensor<1x2xui32>
      %492 = "stablehlo.reshape"(%491) : (tensor<1x2xui32>) -> tensor<2xui32>
      %493 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %494 = "stablehlo.compare"(%arg159, %493) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %495 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %496 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
      %497 = "stablehlo.add"(%495, %496) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %498 = "stablehlo.select"(%494, %497, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %499 = "stablehlo.dynamic_slice"(%arg158, %498) <{slice_sizes = array<i64: 1>}> : (tensor<1xf64>, tensor<i64>) -> tensor<1xf64>
      %500 = "stablehlo.reshape"(%499) : (tensor<1xf64>) -> tensor<f64>
      %501 = "func.call"(%492, %500) <{callee = @None_0}> : (tensor<2xui32>, tensor<f64>) -> tensor<f64>
      %502 = "stablehlo.broadcast_in_dim"(%501) <{broadcast_dimensions = array<i64>}> : (tensor<f64>) -> tensor<1xf64>
      %503 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %504 = "stablehlo.compare"(%arg159, %503) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %505 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %506 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
      %507 = "stablehlo.add"(%505, %506) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %508 = "stablehlo.select"(%504, %507, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %509 = "stablehlo.dynamic_update_slice"(%arg160, %502, %508) : (tensor<1xf64>, tensor<1xf64>, tensor<i64>) -> tensor<1xf64>
      %510 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
      %511 = "stablehlo.add"(%arg159, %510) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      "stablehlo.return"(%arg157, %arg158, %511, %509) : (tensor<1x2xui32>, tensor<1xf64>, tensor<i64>, tensor<1xf64>) -> ()
    }) : (tensor<1x2xui32>, tensor<1xf64>, tensor<i64>, tensor<1xf64>) -> (tensor<1x2xui32>, tensor<1xf64>, tensor<i64>, tensor<1xf64>)
    %483 = "stablehlo.reshape"(%482#3) : (tensor<1xf64>) -> tensor<f64>
    "func.return"(%483) : (tensor<f64>) -> ()
  }) : () -> ()
