"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1x2x3xi8>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1x2x3xi8>, tensor<2x3xi8>)
  ...
  "func.func"() <{function_type = () -> (tensor<1x2x3xi8>, tensor<2x3xi8>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[[3, -1, -1], [0, -5, 0]]]> : tensor<1x2x3xi8>}> : () -> tensor<1x2x3xi8>
    %2 = "stablehlo.constant"() <{value = dense<[[0, 0, 4], [0, -5, 0]]> : tensor<2x3xi8>}> : () -> tensor<2x3xi8>
    "func.return"(%1, %2) : (tensor<1x2x3xi8>, tensor<2x3xi8>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1x2x3xi8>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[[3, -1, 3], [0, -10, 0]]]> : tensor<1x2x3xi8>}> : () -> tensor<1x2x3xi8>
