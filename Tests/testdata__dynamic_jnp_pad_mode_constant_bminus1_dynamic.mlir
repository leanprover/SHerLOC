"builtin.module"() <{sym_name = "jit_fun_flat_jax"}> ({
  ...
  ^bb0(%arg3: tensor<i64>, %arg4: tensor<?x5xf32>):
    %40 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
    %41 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
    %42 = "stablehlo.constant"() <{value = dense<-1> : tensor<i64>}> : () -> tensor<i64>
    %43 = "stablehlo.add"(%arg3, %42) : (tensor<i64>, tensor<i64>) -> tensor<i64>
    %44 = "stablehlo.convert"(%40) : (tensor<i64>) -> tensor<i32>
    %45 = "stablehlo.reshape"(%44) : (tensor<i32>) -> tensor<1xi32>
    %46 = "stablehlo.convert"(%41) : (tensor<i64>) -> tensor<i32>
    %47 = "stablehlo.reshape"(%46) : (tensor<i32>) -> tensor<1xi32>
    %48 = "stablehlo.concatenate"(%45, %47) <{dimension = 0 : i64}> : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %49 = "stablehlo.convert"(%40) : (tensor<i64>) -> tensor<i32>
    %50 = "stablehlo.reshape"(%49) : (tensor<i32>) -> tensor<1xi32>
    %51 = "stablehlo.convert"(%41) : (tensor<i64>) -> tensor<i32>
    %52 = "stablehlo.reshape"(%51) : (tensor<i32>) -> tensor<1xi32>
    %53 = "stablehlo.concatenate"(%50, %52) <{dimension = 0 : i64}> : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %54 = "stablehlo.convert"(%43) : (tensor<i64>) -> tensor<i32>
    %55 = "stablehlo.reshape"(%54) : (tensor<i32>) -> tensor<1xi32>
    %56 = "stablehlo.constant"() <{value = dense<5> : tensor<1xi32>}> : () -> tensor<1xi32>
    %57 = "stablehlo.concatenate"(%55, %56) <{dimension = 0 : i64}> : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %58 = "stablehlo.add"(%53, %57) : (tensor<2xi32>, tensor<2xi32>) -> tensor<2xi32>
    %59 = "stablehlo.constant"() <{value = dense<1> : tensor<1xi32>}> : () -> tensor<1xi32>
    %60 = "stablehlo.constant"() <{value = dense<1> : tensor<1xi32>}> : () -> tensor<1xi32>
    %61 = "stablehlo.concatenate"(%59, %60) <{dimension = 0 : i64}> : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %62 = "stablehlo.real_dynamic_slice"(%arg4, %48, %58, %61) : (tensor<?x5xf32>, tensor<2xi32>, tensor<2xi32>, tensor<2xi32>) -> tensor<?x5xf32>
    %63 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
    %64 = "func.call"(%arg3, %62, %63) <{callee = @_pad}> : (tensor<i64>, tensor<?x5xf32>, tensor<i64>) -> tensor<?x11xf32>
    "func.return"(%64) : (tensor<?x11xf32>) -> ()
  }) : () -> ()
