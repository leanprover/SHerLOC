"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<3xi16>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<3xi16>, tensor<3xi16>)
    ...
    "stablehlo.custom_call"(%6, %5) <{call_target_name = "check.expect_eq", has_side_effect = true}> : (tensor<3xi16>, tensor<3xi16>) -> ()
    "func.return"(%6) : (tensor<3xi16>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> (tensor<3xi16>, tensor<3xi16>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[3, 0, -1]> : tensor<3xi16>}> : () -> tensor<3xi16>
    %2 = "stablehlo.constant"() <{value = dense<[2, 3, 1]> : tensor<3xi16>}> : () -> tensor<3xi16>
    "func.return"(%1, %2) : (tensor<3xi16>, tensor<3xi16>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<3xi16>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[9, 0, -1]> : tensor<3xi16>}> : () -> tensor<3xi16>
