"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<2x3xui64>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %2 = "func.call"() <{callee = @inputs}> : () -> tensor<2x3xi64>
    %3 = "func.call"() <{callee = @expected}> : () -> tensor<2x3xui64>
    %4 = "stablehlo.bitcast_convert"(%2) : (tensor<2x3xi64>) -> tensor<2x3xui64>
    "stablehlo.custom_call"(%4, %3) <{call_target_name = "check.expect_eq", has_side_effect = true}> : (tensor<2x3xui64>, tensor<2x3xui64>) -> ()
    "func.return"(%4) : (tensor<2x3xui64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<2x3xi64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[-4, -1, 0], [-4, -2, 4]]> : tensor<2x3xi64>}> : () -> tensor<2x3xi64>
    "func.return"(%1) : (tensor<2x3xi64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<2x3xui64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[18446744073709551612, 18446744073709551615, 0], [18446744073709551612, 18446744073709551614, 4]]> : tensor<2x3xui64>}> : () -> tensor<2x3xui64>
    "func.return"(%0) : (tensor<2x3xui64>) -> ()
  }) : () -> ()
}) {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} : () -> ()

