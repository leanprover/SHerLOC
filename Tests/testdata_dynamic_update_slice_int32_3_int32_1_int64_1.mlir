"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<3xi32>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %4:3 = "func.call"() <{callee = @inputs}> : () -> (tensor<3xi32>, tensor<1xi32>, tensor<1xi64>)
    %5 = "func.call"() <{callee = @expected}> : () -> tensor<3xi32>
    ...
    %13 = "stablehlo.dynamic_update_slice"(%4#0, %4#1, %12) : (tensor<3xi32>, tensor<1xi32>, tensor<i64>) -> tensor<3xi32>
    "stablehlo.custom_call"(%13, %5) <{call_target_name = "check.expect_eq", has_side_effect = true}> : (tensor<3xi32>, tensor<3xi32>) -> ()
    "func.return"(%13) : (tensor<3xi32>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> (tensor<3xi32>, tensor<1xi32>, tensor<1xi64>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[-2, 4, -1]> : tensor<3xi32>}> : () -> tensor<3xi32>
    %2 = "stablehlo.constant"() <{value = dense<-1> : tensor<1xi32>}> : () -> tensor<1xi32>
    %3 = "stablehlo.constant"() <{value = dense<1> : tensor<1xi64>}> : () -> tensor<1xi64>
    "func.return"(%1, %2, %3) : (tensor<3xi32>, tensor<1xi32>, tensor<1xi64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<3xi32>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[-2, -1, -1]> : tensor<3xi32>}> : () -> tensor<3xi32>
