"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1xbf16>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %3:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<3xbf16>, tensor<1xi64>)
    %4 = "func.call"() <{callee = @expected}> : () -> tensor<1xbf16>
    ...
    %12 = "stablehlo.dynamic_slice"(%3#0, %11) <{slice_sizes = array<i64: 1>}> : (tensor<3xbf16>, tensor<i64>) -> tensor<1xbf16>
    "stablehlo.custom_call"(%12, %4) <{call_target_name = "check.expect_close", has_side_effect = true}> : (tensor<1xbf16>, tensor<1xbf16>) -> ()
    "func.return"(%12) : (tensor<1xbf16>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> (tensor<3xbf16>, tensor<1xi64>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[-4.000000e+00, 7.539060e-01, -1.093750e+00]> : tensor<3xbf16>}> : () -> tensor<3xbf16>
    %2 = "stablehlo.constant"() <{value = dense<1> : tensor<1xi64>}> : () -> tensor<1xi64>
    "func.return"(%1, %2) : (tensor<3xbf16>, tensor<1xi64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1xbf16>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<7.539060e-01> : tensor<1xbf16>}> : () -> tensor<1xbf16>
