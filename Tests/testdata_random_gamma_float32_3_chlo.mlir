"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<3xf64>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %531 = "func.call"() <{callee = @inputs}> : () -> tensor<3xf32>
    %532 = "func.call"() <{callee = @expected}> : () -> tensor<3xf64>
    %533 = "func.call"(%531) <{callee = @"<lambda>"}> : (tensor<3xf32>) -> tensor<3xf64>
    "stablehlo.custom_call"(%533, %532) <{call_target_name = "check.expect_close", has_side_effect = true}> : (tensor<3xf64>, tensor<3xf64>) -> ()
    "func.return"(%533) : (tensor<3xf64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<3xf32>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %530 = "stablehlo.constant"() <{value = dense<[-2.83901238, -2.29982233, -4.13669348]> : tensor<3xf32>}> : () -> tensor<3xf32>
    "func.return"(%530) : (tensor<3xf32>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<3xf64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %529 = "stablehlo.constant"() <{value = dense<0xFFF8000000000000> : tensor<3xf64>}> : () -> tensor<3xf64>
    "func.return"(%529) : (tensor<3xf64>) -> ()
  }) : () -> ()
  "func.func"() <{arg_attrs = [{mhlo.layout_mode = "default"}], function_type = (tensor<3xf32>) -> tensor<3xf64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "<lambda>", sym_visibility = "private"}> ({
  ^bb0(%arg165: tensor<3xf32>):
    %517 = "stablehlo.constant"() <{value = dense<42> : tensor<i64>}> : () -> tensor<i64>
    %518 = "stablehlo.constant"() <{value = dense<32> : tensor<i64>}> : () -> tensor<i64>
    %519 = "stablehlo.shift_right_logical"(%517, %518) : (tensor<i64>, tensor<i64>) -> tensor<i64>
    %520 = "stablehlo.convert"(%519) : (tensor<i64>) -> tensor<ui32>
    %521 = "stablehlo.broadcast_in_dim"(%520) <{broadcast_dimensions = array<i64>}> : (tensor<ui32>) -> tensor<1xui32>
    %522 = "stablehlo.constant"() <{value = dense<4294967295> : tensor<ui32>}> : () -> tensor<ui32>
    %523 = "stablehlo.convert"(%522) : (tensor<ui32>) -> tensor<i64>
    %524 = "stablehlo.and"(%517, %523) : (tensor<i64>, tensor<i64>) -> tensor<i64>
    %525 = "stablehlo.convert"(%524) : (tensor<i64>) -> tensor<ui32>
    %526 = "stablehlo.broadcast_in_dim"(%525) <{broadcast_dimensions = array<i64>}> : (tensor<ui32>) -> tensor<1xui32>
    %527 = "stablehlo.concatenate"(%521, %526) <{dimension = 0 : i64}> : (tensor<1xui32>, tensor<1xui32>) -> tensor<2xui32>
    %528 = "func.call"(%527, %arg165) <{callee = @_gamma}> : (tensor<2xui32>, tensor<3xf32>) -> tensor<3xf64>
    "func.return"(%528) : (tensor<3xf64>) -> ()
  }) : () -> ()
  "func.func"() <{arg_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], function_type = (tensor<2xui32>, tensor<3xf32>) -> tensor<3xf64>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "_gamma", sym_visibility = "private"}> ({
  ^bb0(%arg155: tensor<2xui32>, %arg156: tensor<3xf32>):
    %479 = "stablehlo.convert"(%arg156) : (tensor<3xf32>) -> tensor<3xf64>
    %480 = "stablehlo.reshape"(%arg155) : (tensor<2xui32>) -> tensor<1x2xui32>
    %481 = "func.call"(%480) <{callee = @_threefry_split}> : (tensor<1x2xui32>) -> tensor<1x3x2xui32>
    %482 = "stablehlo.reshape"(%481) : (tensor<1x3x2xui32>) -> tensor<3x2xui32>
    %483 = "stablehlo.constant"() <{value = dense<0.000000e+00> : tensor<f64>}> : () -> tensor<f64>
    %484 = "stablehlo.broadcast_in_dim"(%483) <{broadcast_dimensions = array<i64>}> : (tensor<f64>) -> tensor<3xf64>
    %485 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
    %486:4 = "stablehlo.while"(%482, %479, %485, %484) ({
    ^bb0(%arg161: tensor<3x2xui32>, %arg162: tensor<3xf64>, %arg163: tensor<i64>, %arg164: tensor<3xf64>):
      %515 = "stablehlo.constant"() <{value = dense<3> : tensor<i64>}> : () -> tensor<i64>
      %516 = "stablehlo.compare"(%arg163, %515) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      "stablehlo.return"(%516) : (tensor<i1>) -> ()
    }, {
    ^bb0(%arg157: tensor<3x2xui32>, %arg158: tensor<3xf64>, %arg159: tensor<i64>, %arg160: tensor<3xf64>):
      %487 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %488 = "stablehlo.compare"(%arg159, %487) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %489 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %490 = "stablehlo.constant"() <{value = dense<3> : tensor<i64>}> : () -> tensor<i64>
      %491 = "stablehlo.add"(%489, %490) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %492 = "stablehlo.select"(%488, %491, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %493 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %494 = "stablehlo.dynamic_slice"(%arg157, %492, %493) <{slice_sizes = array<i64: 1, 2>}> : (tensor<3x2xui32>, tensor<i64>, tensor<i64>) -> tensor<1x2xui32>
      %495 = "stablehlo.reshape"(%494) : (tensor<1x2xui32>) -> tensor<2xui32>
      %496 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %497 = "stablehlo.compare"(%arg159, %496) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %498 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %499 = "stablehlo.constant"() <{value = dense<3> : tensor<i64>}> : () -> tensor<i64>
      %500 = "stablehlo.add"(%498, %499) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %501 = "stablehlo.select"(%497, %500, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %502 = "stablehlo.dynamic_slice"(%arg158, %501) <{slice_sizes = array<i64: 1>}> : (tensor<3xf64>, tensor<i64>) -> tensor<1xf64>
      %503 = "stablehlo.reshape"(%502) : (tensor<1xf64>) -> tensor<f64>
      %504 = "func.call"(%495, %503) <{callee = @None_0}> : (tensor<2xui32>, tensor<f64>) -> tensor<f64>
      %505 = "stablehlo.broadcast_in_dim"(%504) <{broadcast_dimensions = array<i64>}> : (tensor<f64>) -> tensor<1xf64>
      %506 = "stablehlo.constant"() <{value = dense<0> : tensor<i64>}> : () -> tensor<i64>
      %507 = "stablehlo.compare"(%arg159, %506) <{compare_type = #stablehlo<comparison_type SIGNED>, comparison_direction = #stablehlo<comparison_direction LT>}> : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %508 = "stablehlo.convert"(%arg159) : (tensor<i64>) -> tensor<i64>
      %509 = "stablehlo.constant"() <{value = dense<3> : tensor<i64>}> : () -> tensor<i64>
      %510 = "stablehlo.add"(%508, %509) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      %511 = "stablehlo.select"(%507, %510, %arg159) : (tensor<i1>, tensor<i64>, tensor<i64>) -> tensor<i64>
      %512 = "stablehlo.dynamic_update_slice"(%arg160, %505, %511) : (tensor<3xf64>, tensor<1xf64>, tensor<i64>) -> tensor<3xf64>
      %513 = "stablehlo.constant"() <{value = dense<1> : tensor<i64>}> : () -> tensor<i64>
      %514 = "stablehlo.add"(%arg159, %513) : (tensor<i64>, tensor<i64>) -> tensor<i64>
      "stablehlo.return"(%arg157, %arg158, %514, %512) : (tensor<3x2xui32>, tensor<3xf64>, tensor<i64>, tensor<3xf64>) -> ()
    }) : (tensor<3x2xui32>, tensor<3xf64>, tensor<i64>, tensor<3xf64>) -> (tensor<3x2xui32>, tensor<3xf64>, tensor<i64>, tensor<3xf64>)
    "func.return"(%486#3) : (tensor<3xf64>) -> ()
  }) : () -> ()
