"builtin.module"() <{sym_name = "jit_main"}> ({
  ...
  "func.func"() <{function_type = () -> (tensor<1x2x3xi8>, tensor<1x3xi8>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[[7, 3, 1], [-1, 0, -1]]]> : tensor<1x2x3xi8>}> : () -> tensor<1x2x3xi8>
    %2 = "stablehlo.constant"() <{value = dense<[[6, -2, 0]]> : tensor<1x3xi8>}> : () -> tensor<1x3xi8>
  ...
  "func.func"() <{function_type = () -> tensor<1x2x3xi8>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[[7, 3, 1], [5, -2, -1]]]> : tensor<1x2x3xi8>}> : () -> tensor<1x2x3xi8>
