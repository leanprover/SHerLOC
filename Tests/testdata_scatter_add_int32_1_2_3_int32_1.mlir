"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1x2x3xi32>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1x2x3xi32>, tensor<1xi32>)
    ...
    }) : (tensor<1x2x3xi32>, tensor<2xi64>, tensor<1xi32>) -> tensor<1x2x3xi32>
  ...
  "func.func"() <{function_type = () -> (tensor<1x2x3xi32>, tensor<1xi32>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[[-1, 0, 2], [6, 0, 5]]]> : tensor<1x2x3xi32>}> : () -> tensor<1x2x3xi32>
    %2 = "stablehlo.constant"() <{value = dense<0> : tensor<1xi32>}> : () -> tensor<1xi32>
    "func.return"(%1, %2) : (tensor<1x2x3xi32>, tensor<1xi32>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1x2x3xi32>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[[-1, 0, 2], [6, 0, 5]]]> : tensor<1x2x3xi32>}> : () -> tensor<1x2x3xi32>
