"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<4x6xi16>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %3:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<4x3xi16>, tensor<3x6xui8>)
    %4 = "func.call"() <{callee = @expected}> : () -> tensor<4x6xi16>
    %5 = "stablehlo.convert"(%3#0) : (tensor<4x3xi16>) -> tensor<4x3xi16>
    %6 = "stablehlo.convert"(%3#1) : (tensor<3x6xui8>) -> tensor<3x6xi16>
  ...
  "func.func"() <{function_type = () -> (tensor<4x3xi16>, tensor<3x6xui8>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[1, 1, 2], [0, -4, 0], [0, -4, -4], [0, 0, 0]]> : tensor<4x3xi16>}> : () -> tensor<4x3xi16>
    %2 = "stablehlo.constant"() <{value = dense<[[0, 3, 2, 0, 1, 1], [2, 2, 0, 0, 3, 3], [2, 0, 1, 2, 5, 1]]> : tensor<3x6xui8>}> : () -> tensor<3x6xui8>
    "func.return"(%1, %2) : (tensor<4x3xi16>, tensor<3x6xui8>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<4x6xi16>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[6, 5, 4, 4, 14, 6], [-8, -8, 0, 0, -12, -12], [-16, -8, -4, -8, -32, -16], [0, 0, 0, 0, 0, 0]]> : tensor<4x6xi16>}> : () -> tensor<4x6xi16>
