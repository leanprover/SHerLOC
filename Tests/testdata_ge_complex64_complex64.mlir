"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<i1>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %10 = "stablehlo.compare"(%8, %9) <{compare_type = #stablehlo<comparison_type FLOAT>, comparison_direction = #stablehlo<comparison_direction GE>}> : (tensor<f32>, tensor<f32>) -> tensor<i1>
    %11 = "stablehlo.real"(%3#0) : (tensor<complex<f32>>) -> tensor<f32>
    %12 = "stablehlo.real"(%3#1) : (tensor<complex<f32>>) -> tensor<f32>
    %13 = "stablehlo.compare"(%11, %12) <{compare_type = #stablehlo<comparison_type FLOAT>, comparison_direction = #stablehlo<comparison_direction GE>}> : (tensor<f32>, tensor<f32>) -> tensor<i1>
  ...
  "func.func"() <{function_type = () -> (tensor<complex<f32>>, tensor<complex<f32>>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<(6.24502802,0.706214964)> : tensor<complex<f32>>}> : () -> tensor<complex<f32>>
    %2 = "stablehlo.constant"() <{value = dense<(4.1928339,-0.170854881)> : tensor<complex<f32>>}> : () -> tensor<complex<f32>>
