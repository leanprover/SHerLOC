"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1x2x3xui8>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1x2x3xui8>, tensor<2x3xui8>)
    ...
    }) : (tensor<1x2x3xui8>, tensor<1xi64>, tensor<2x3xui8>) -> tensor<1x2x3xui8>
  ...
  "func.func"() <{function_type = () -> (tensor<1x2x3xui8>, tensor<2x3xui8>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<[[[4, 4, 1], [5, 0, 0]]]> : tensor<1x2x3xui8>}> : () -> tensor<1x2x3xui8>
    %2 = "stablehlo.constant"() <{value = dense<[[3, 3, 4], [1, 3, 5]]> : tensor<2x3xui8>}> : () -> tensor<2x3xui8>
    "func.return"(%1, %2) : (tensor<1x2x3xui8>, tensor<2x3xui8>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1x2x3xui8>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<[[[12, 12, 4], [5, 0, 0]]]> : tensor<1x2x3xui8>}> : () -> tensor<1x2x3xui8>
