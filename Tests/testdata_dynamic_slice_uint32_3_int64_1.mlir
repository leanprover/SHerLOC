"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1xui32>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    %3:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<3xui32>, tensor<1xi64>)
    %4 = "func.call"() <{callee = @expected}> : () -> tensor<1xui32>
    ...
    %12 = "stablehlo.dynamic_slice"(%3#0, %11) <{slice_sizes = array<i64: 1>}> : (tensor<3xui32>, tensor<i64>) -> tensor<1xui32>
    "stablehlo.custom_call"(%12, %4) <{call_target_name = "check.expect_eq", has_side_effect = true}> : (tensor<1xui32>, tensor<1xui32>) -> ()
    "func.return"(%12) : (tensor<1xui32>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> (tensor<3xui32>, tensor<1xi64>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<0> : tensor<3xui32>}> : () -> tensor<3xui32>
    %2 = "stablehlo.constant"() <{value = dense<1> : tensor<1xi64>}> : () -> tensor<1xi64>
    "func.return"(%1, %2) : (tensor<3xui32>, tensor<1xi64>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1xui32>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<0> : tensor<1xui32>}> : () -> tensor<1xui32>
