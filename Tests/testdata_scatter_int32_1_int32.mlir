"builtin.module"() <{sym_name = "jit_main"}> ({
  "func.func"() <{function_type = () -> tensor<1xi32>, res_attrs = [{jax.result_info = "", mhlo.layout_mode = "default"}], sym_name = "main", sym_visibility = "public"}> ({
    ...
    %4:2 = "func.call"() <{callee = @inputs}> : () -> (tensor<1xi32>, tensor<i32>)
    ...
    }) : (tensor<1xi32>, tensor<1xi64>, tensor<i32>) -> tensor<1xi32>
  ...
  "func.func"() <{function_type = () -> (tensor<1xi32>, tensor<i32>), res_attrs = [{mhlo.layout_mode = "default"}, {mhlo.layout_mode = "default"}], sym_name = "inputs", sym_visibility = "private"}> ({
    %1 = "stablehlo.constant"() <{value = dense<4> : tensor<1xi32>}> : () -> tensor<1xi32>
    %2 = "stablehlo.constant"() <{value = dense<4> : tensor<i32>}> : () -> tensor<i32>
    "func.return"(%1, %2) : (tensor<1xi32>, tensor<i32>) -> ()
  }) : () -> ()
  "func.func"() <{function_type = () -> tensor<1xi32>, res_attrs = [{mhlo.layout_mode = "default"}], sym_name = "expected", sym_visibility = "private"}> ({
    %0 = "stablehlo.constant"() <{value = dense<4> : tensor<1xi32>}> : () -> tensor<1xi32>
