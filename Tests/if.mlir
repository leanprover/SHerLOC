"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "if_ops_true_branch"}> ({
    ...
    %5:2 = "stablehlo.if"(%4) ({
      %7 = "stablehlo.constant"() <{value = dense<0> : tensor<2xi64>}> : () -> tensor<2xi64>
      "stablehlo.return"(%7, %7) : (tensor<2xi64>, tensor<2xi64>) -> ()
    }, {
    ...
    "check.expect_eq_const"(%5#0) <{value = dense<0> : tensor<2xi64>}> : (tensor<2xi64>) -> ()
    "check.expect_eq_const"(%5#1) <{value = dense<0> : tensor<2xi64>}> : (tensor<2xi64>) -> ()
    "func.return"() : () -> ()
  }) : () -> ()
